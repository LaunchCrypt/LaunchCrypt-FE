...
function SwapHeader({
  ...
  return (
    <>
      {showWalletWarning && <Modal isVisible={showWalletWarning}
        onClose={() => setShowWalletWarning(false)}
        children={<WalletWarning closeModal={() => setShowWalletWarning(false)} />} />
      }
      <div
        className={`fixed top-0 right-0 h-full w-96 bg-gray-900 shadow-xl transform transition-transform duration-300 ease-in-out ${showCreatePool ? 'translate-x-0 z-50' : 'translate-x-full'
          }`}
      >
